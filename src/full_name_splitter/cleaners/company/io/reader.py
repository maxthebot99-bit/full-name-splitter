...
def guess_name_column(columns: list[str]) -> str | None:
    """Heuristic: pick a column whose name looks like a company-name field."""
    hints = ["company", "account", "organization", "organisation", "business", "name", "brand"]
    lowered = {c: c.lower() for c in columns}
    # exact matches first
    for c, low in lowered.items():
        for h in hints:
            if low == h or low == f"{h} name" or low == f"{h}_name":
                return c
    # partial
    for c, low in lowered.items():
        for h in hints:
