"""Encoding/delimiter sniffer + Excel detector. Copy of company/io/format_detect.py.

Kept per-cleaner-package per the existing isolation pattern (each
``cleaners/<kind>/`` is self-contained). If a shared utility module is
introduced later, both can re-target.
"""
