...
``full_name_splitter.cleaners.{fullname}``; this worker dispatches to the
right one based on ``kind``. (Forked from cleaners-hub which supported
company / name / address; in the splitter the only kind is ``fullname``.)
...
SUPPORTED_KINDS = ("fullname",)


def _modules_for(kind: str):
    """Return (pipeline, io_reader, io_writer, types, XAIProvider, errors, config) for the kind.

    Only ``fullname`` is supported in the splitter. The original
    cleaners-hub had branches for company / name / address; those kinds
    were removed in Phase B.
    """
...
def run_worker(
    ...
    *,
    ...
    ``secondary_column`` is unused in the splitter (kept in the signature
    for compat with the existing /api/run body shape). The only supported
    kind is ``fullname``.
    """
...
def spawn_run(session: Session, *, column: str, row_limit: int | None,
    ...
    ``secondary_column`` is unused in the splitter (kept in the signature
    for compat with the existing /api/run body shape).
    """
    t = threading.Thread(
        target=run_worker,
        kwargs={
