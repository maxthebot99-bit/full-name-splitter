# full-name-splitter — Security Posture
...
All traffic to `full-name-splitter.maxcommandcenter.com` is gated by **Cloudflare Access** at the edge. The wildcard policy on `*.maxcommandcenter.com` enforces SSO via Benchmark M365 identity. The origin trusts the `Cf-Access-Authenticated-User-Email` header injected by `cloudflared` and uses it for: per-user ownership scoping, rate-limit bucketing, and audit-log attribution. Origin binds to `127.0.0.1:8181`, never to a public interface.

Admin operations (settings PUT, cross-user run history, model whitelist edits) are gated by an in-code allowlist (`ADMIN_EMAILS` in `src/full_name_splitter/main.py`). The allowlist is intentionally hardcoded, not env-driven, to keep admin gating outside the blast radius of a compromised `.env`.

## Secret management (SOC 2 CC6.7 / ISO 27001 A.8.24)

All production credentials (xAI, OpenRouter, Resend) are stored via **systemd-creds** as encrypted files in `/etc/credstore.encrypted/`. At unit start they are decrypted into `/run/credentials/full-name-splitter.service/<name>` (tmpfs, RAM-only) and exposed to the app via file-path environment variables (`XAI_API_KEY_FILE`, etc.). The credential VALUES are never in env strings, never in process command lines, and never readable by other users on the host.

The app's audit logger has a `RedactingFilter` (`src/full_name_splitter/audit.py`) that strips xAI key patterns from every log record before emit — defense in depth in case a key ever leaks into a log line via stack trace or httpx URL error.

## Data flow + retention (SOC 2 P4.1 / ISO 27001 A.5.14, A.8.10)

User-uploaded CSVs are written to per-session UUIDv4 directories under `/var/lib/full-name-splitter/`. The runtime stores: the upload itself, intermediate cleaning artifacts (stage1.csv, etc.), the cleaned download. Files are scoped per-session per-user and only accessible to their owner via authenticated downloads. No external persistence beyond the VPS.
...
Hard daily spend cap: `$100/day` as a code constant in `src/full_name_splitter/spend.py`. The admin UI can lower the cap but cannot raise the ceiling without a code change + deploy.

## Network egress (SOC 2 CC6.6 / ISO 27001 A.8.20, A.8.21)

The systemd unit (`deploy/full-name-splitter.service`) enforces `IPAddressDeny=` for RFC1918, link-local + cloud metadata (`169.254.0.0/16`), Tailscale CGNAT (`100.64.0.0/10`), and IPv6 link-local + ULA. This blocks SSRF attempts via user-supplied CSV cells (`address` tab `website_url` column) at the network layer.

The application layer adds `_is_safe_url()` (`src/full_name_splitter/cleaners/address/fetch.py`) which resolves URLs via `socket.getaddrinfo` and rejects any IP in private/loopback/link-local/multicast/reserved ranges before issuing the HTTP request. Closes the loopback path that the systemd layer intentionally leaves open for unit-internal health checks. Also re-validates redirect targets after `allow_redirects=True`.

## Application hardening (SOC 2 CC6.6 / ISO 27001 A.8.21)

The systemd unit applies 14 hardening directives:
- `NoNewPrivileges=true`
- `ProtectSystem=strict` + `ReadWritePaths=/var/lib/full-name-splitter`
...
- `StateDirectory=full-name-splitter` + `RuntimeDirectory=full-name-splitter`
...
Every `/api/*` route except `/api/health` enforces an `X-Requested-With: full-name-splitter` header via `CSRFCheckMiddleware`. Browser-originated requests must originate from the app's own JS to attach the header; cross-origin requests cannot.
...
Source: `github.com/maxthebot99-bit/full-name-splitter` (private). Production deploys auto-pull from `origin/main` every 60s via `full-name-splitter-autodeploy.timer` running `deploy/auto-deploy.sh`.
...
Structured audit log written via Python `logging` to journald (`SyslogIdentifier=full-name-splitter`). The `RedactingFilter` strips secret patterns before emit. `_safe_500()` returns a generic 500 message to clients and only logs the real exception server-side.
...
| Confidential | User-uploaded CSVs (Benchmark prospect data), cleaned outputs | `/var/lib/full-name-splitter/<session-uuid>/` (per-user scope) |
| Internal | journald logs, spend.sqlite3 | `/var/lib/full-name-splitter/` |
...
2. systemd unit at `/etc/systemd/system/full-name-splitter.service` — confirm `User=www-data`, `NoNewPrivileges`, `ProtectSystem=strict`, `IPAddressDeny` rules, `LoadCredentialEncrypted` for all three keys
